...
router.get("/kontenery", async (req, res) => {
  ...
  try {
    ...
    res.status(200).json({
      ...
      _links: {
        self: { href: "http://localhost:3000/api/kontenery", method: "GET" },
        addContainer: {
          href: "http://localhost:3000/api/kontenery/add",
          method: "POST",
        },
      },
    });
...
router.get("/kontenery/:id", async (req, res) => {
  res.setHeader("Cache-Control", "no-store");
  res.setHeader("X-Powered-By", "Express");
  res.setHeader("Content-Type", "application/json");
  ...
  try {
    ...
    const container = containers.find(
      (c) => c.kontener.numer_zlecenia === orderId
    );

    if (!container) {
      return res
        .status(404)
        .send("Kontener o podanym numerze zlecenia nie został znaleziony.");
    }
    ...
    res.status(200).json({
      ...
      _links: {
        self: {
          href: `http://localhost:3000/api/kontenery/${orderId}`,
          method: "GET",
        },
        update: {
          href: `http://localhost:3000/api/kontenery/update/${orderId}`,
          method: "PATCH",
        },
        delete: {
          href: `http://localhost:3000/api/kontenery/delete/${orderId}`,
          method: "DELETE",
        },
        allContainers: {
          href: "http://localhost:3000/api/kontenery",
          method: "GET",
        },
      },
    });
...
router.post("/kontenery/add", async (req, res) => {
  res.setHeader("Content-Type", "application/json");
  ...
  if (
    !newContainer.kontener?.numer_zlecenia ||
    !newContainer.kontener?.numer_kontenera
  ) {
    return res
      .status(400)
      .send("Brakuje wymaganych pól: numer_zlecenia, numer_kontenera");
  }
  ...
  try {
    ...
    const existingContainer = containers.find(
      (c) => c.kontener.numer_zlecenia === newContainer.kontener.numer_zlecenia
    );

    if (existingContainer) {
      return res
        .status(400)
        .send("Kontener o podanym numerze zlecenia już istnieje.");
    }
    ...
    res.status(201).json({
      ...
      _links: {
        self: {
          href: `http://localhost:3000/api/kontenery/${newContainer.kontener.numer_zlecenia}`,
          method: "GET",
        },
        allContainers: {
          href: "http://localhost:3000/api/kontenery",
...
router.patch("/kontenery/update/:id", async (req, res) => {
  res.setHeader("Content-Type", "application/json");
  ...
  try {
    ...
    if (containerIndex === -1) {
      return res
        .status(404)
        .send("Kontener o podanym numerze zlecenia nie istnieje.");
    }
...
router.put("/kontenery/update/:id", async (req, res) => {
  res.setHeader("Content-Type", "application/json");
  res.setHeader("X-Action", "Container Full Update");
  res.setHeader("X-Powered-By", "Express");
  ...
  if (
    !updatedContainer.kontener?.numer_zlecenia ||
    !updatedContainer.kontener?.numer_kontenera
  ) {
    return res
      .status(400)
      .send("Brakuje wymaganych pól: numer_zlecenia, numer_kontenera");
  ...
  try {
    ...
    console.error("Błąd podczas pełnej aktualizacji kontenera:", error);
    res.status(500).send("Błąd przy pełnej aktualizacji kontenera.");
  }
});

router.delete("/kontenery/delete/:id", async (req, res) => {
  res.setHeader("Content-Type", "application/json");
  ...
  try {
    ...
    if (containerIndex === -1) {
      return res
        .status(404)
        .send(`Kontener o numerze zlecenia ${orderId} nie został znaleziony.`);
    }
