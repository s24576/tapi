...
router.get('/players', async (req, res) => {
    ...
    try {
        ...
        if (req.query.unit) {
            ...
            const filteredProducts = products.filter(p => p.towar.jednostka === unit);

            if (filteredProducts.length > 0) {
                return res.status(200).json({
                    products: filteredProducts,
                    _links: {
                        self: { href: `http://localhost:3000/api/players?unit=${unit}`, method: 'GET' },
                        allProducts: { href: 'http://localhost:3000/api/players', method: 'GET' }
                    }
                });
            } else {
                return res.status(404).send(`Nie znaleziono towarów o jednostce: ${unit}`);
            }
        }
        ...
        res.status(200).json({
            ...
            _links: {
                ...
                addProduct: { href: 'http://localhost:3000/api/players/add', method: 'POST' }
            }
        });
    } catch (error) {
...
router.get('/players/:id', async (req, res) => {
    ...
    try {
        ...
        if (!product) {
            return res.status(404).send('Towar o podanym numerze nie został znaleziony.');
        }

        res.status(200).json({
            product,
            _links: {
                self: { href: `http://localhost:3000/api/players/${productId}`, method: 'GET' },
                update: { href: `http://localhost:3000/api/players/update/${productId}`, method: 'PATCH' },
                delete: { href: `http://localhost:3000/api/players/delete/${productId}`, method: 'DELETE' },
                allProducts: { href: 'http://localhost:3000/api/players', method: 'GET' }
            }
        });
    } catch (error) {
...
router.post('/players/add', async (req, res) => {
    ...
    if (!newProduct.towar?.numer_towaru || !newProduct.towar?.nazwa || 
        !newProduct.towar?.ilość || !newProduct.towar?.jednostka) {
        return res.status(400).send('Brakuje wymaganych pól: numer_towaru, nazwa, ilość lub jednostka.');
    }
    ...
    try {
        ...
        const existingProduct = products.find(p => 
            p.towar.numer_towaru === newProduct.towar.numer_towaru
        );
        ...
        res.status(201).json({
            ...
            _links: {
                self: { href: `http://localhost:3000/api/players/${newProduct.towar.numer_towaru}`, method: 'GET' },
                allProducts: { href: 'http://localhost:3000/api/players', method: 'GET' }
            }
        });
    } catch (error) {
...
router.patch('/players/update/:id', async (req, res) => {
    ...
    try {
        const products = await loadProductsFromJSON();
        const productIndex = products.findIndex(p => p.towar.numer_towaru === productId);
